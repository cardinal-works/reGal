...
contract RegalAuction is IERC721Receiver {
    // Variables
    ...
    function addOperator(address operator) public {
        require(
            msg.sender == owner,
            "Only contract creator is allowed to do this."
        );
    ...
    function force_remove_bid(address NFTContract, uint256 NFTId) public {
        ...
        balance[current_bidder[NFTContract][NFTId]] += current_auction_price[
            NFTContract
        ][NFTId];
        emit ForceRemoval(
            msg.sender,
            NFTContract,
            NFTId,
            current_bidder[NFTContract][NFTId]
        );
    }
    ...
    function startAuction(
        address NFTContract,
        uint256 NFTId,
        uint256 lowest_price
    ) public {
        require(
            msg.sender == _ownership[NFTContract][NFTId],
            "Permission denied"
        );
        // 1. Set lowest bid
    ...
    function bid(address NFTContract, uint256 NFTId) public payable {
        require(
            msg.value > current_auction_price[NFTContract][NFTId],
            "Must bid higher"
        );
    ...
    function claimAuction(address NFTContract, uint256 NFTId) public {
        require(
            block.number > bid_end_time[NFTContract][NFTId],
        ...
        balance[_ownership[NFTContract][NFTId]] += current_auction_price[
            NFTContract
        ][NFTId];
    ...
    function onERC721Received(
        ...
        require(
            _allowed_to_exchange[msg.sender],
            "Token not approved for auction"
        );
        // Set token ownership
